...
##Chris Hale - altered methratio.py script to incorporate the "Steve filter"
...
parser.add_option("-S", "--Steve", action="store_true", dest="Steve_on", help="activate the Steve filter!!!", default=False)
...
for line in open(options.reffile):
    if line[0] == '>': 
        if any(cr): 
            if len(options.chroms) == 0 or cr in options.chroms: ref[cr] = seq.upper()#Adding current chromosome to array "ref" before moving to the next
...
for cr in ref:
    meth[cr] = array.array('L', [0]) * len(ref[cr])
    depth[cr] = array.array('L', [0]) * len(ref[cr])
    if options.rm_dup: coverage[cr] = array.array('B', [0]) * len(ref[cr])
    if options.CT_SNP > 0:
        meth1[cr] = array.array('L', [0]) * len(ref[cr])
        depth1[cr] = array.array('L', [0]) * len(ref[cr])

options.chroms = set(ref.keys())

BS_conversion = {'+': ('C','T','G','A'), '-': ('G','A','C','T')}
nmap = 0
Steve_filter_total=0
Steve_filter_flag=False
for infile in infiles:
    ...
    for line in fin:
        ...
        seq, strand, cr, pos = map_info #we've now separated the alignment data for a read
        depthcr = depth[cr]
        if pos + len(seq) > len(depthcr): continue #not sure what's going on here, making sure the read is on the chromosome?
        nmap += 1
        methcr = meth[cr]
        refseq = ref[cr][pos:pos+len(seq)] #here is a substring of the chromosome
        match, convert, rc_match, rc_convert = BS_conversion[strand] #Ah,  this is just a way of setting the values of these four scalars to bp values
        if options.Steve_on:
            if strand=="+":
                asymetric_cytosines = re.finditer(r"C[ATC][ATC]",refseq)
            elif strand=="-":
                asymetric_cytosines = re.finditer(r"G[TAG][TAG]",refseq)
            asymetric_list=[m.start(0) for m in asymetric_cytosines]
            Steve_filter_flag=False
            Steve_filter=0
            for asymetic_pos in asymetric_list:
                if seq[asymetic_pos] == convert:
                    Steve_filter=0
                elif seq[asymetic_pos] == match: #if not converted
                    Steve_filter+=1
                    if Steve_filter==3:
                        Steve_filter_flag=True
            if Steve_filter_flag==True:
                Steve_filter_total+=1
        if not Steve_filter_flag:
            index = refseq.find(match) #find all the reference cytosines
            #print '%d,%d' %(index,pos)
            #print '%s,%s' %(seq,convert)
            while index >= 0:
                if depthcr[pos+index] < 4294967295: #Don't understand this, is this a cap on coverage?
                    if seq[index] == convert:
                        depthcr[pos+index] += 1 #add 1 to total sequencing depth of array
                    elif seq[index] == match: #if converted, add one to meth array and depth array
                        methcr[pos+index] += 1
                        depthcr[pos+index] += 1
            ...
            while index >= 0:
                if depthcr1[pos+index] < 4294967295:
...
if options.combine_CpG:
    ...
    for cr in depth:
        ...
        pos = refcr.find('CG')
        while pos >= 0:
            if depthcr[pos] + depthcr[pos+1] <= 4294967295:
            ...
            methcr[pos+1] = 0
            if options.CT_SNP > 0:
                if depthcr1[pos] + depthcr1[pos+1] <= 4294967295:
...
fout.close()
disp('done.')
if nc > 0: print 'total %d valid mappings, %d reads tossed by Steve filter, %d covered cytosines, average coverage: %.2f fold.' % (nmap, Steve_filter_total,nc, float(nd)/nc)
